...
def get_window(window_type: str,
    ...
    """
    window = getattr(torch, f"{window_type}_window")(window_size, device=device, periodic=periodic)
...
def unframe_signal(x: Tensor, frame_shift: int) -> Tensor:
    ...
    return torch.nn.functional.fold(x, output_size=(n_sample, 1), kernel_size=(frame_size, 1), stride=(frame_shift, 1)).squeeze(-1)


def frame_signal(x: Tensor, frame_size: int, frame_shift: int) -> Tensor:
    """Frame a signal with given frame size and frame shift.
    The first frame starts from 0. When the signal x is not long enough to fill a frame, the sampling points are dropped.
    ...
    Returns:
        framed_x (Tensor):
        [n_batch, frame_size, (n_sample - frame_size) // frame_shift]
    """
    return torch.nn.functional.unfold(x.unsqueeze(-1), kernel_size=(frame_size, 1), stride=(frame_shift, 1))

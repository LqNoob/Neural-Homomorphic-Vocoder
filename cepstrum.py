...
def complex_cepstrum_to_fft(ccep: Tensor, fft_size: int, dim: int = -1) -> Tuple[Tensor, Tensor, Tensor]:
    """Convert complex cepstrums to its corresponding fft.
    Args:
        ccep: ccep.size(dim) is ccep_size.
        fft_size: Target fft_size, should be substantially larger than ccep_size to avoid time wraping error.
        dim: Dimension storing the complex cepstrum. Defaults to -1.
    Returns:
        X: FFT of the impulse response x
        log(|X|): Log scale magnitude responses at FFT sampling points.
    ...
    magnitude_responses = log_magnitude_responses.exp() # TODO
...
def complex_cepstrum_to_imp(ccep: Tensor, fft_size: int, dim: int = -1) -> Tensor:
    """Convert complex cepstrums to corresponding magnitude responses, phase responses, and impulse responses.
    Args:
        ccep: [ccep_size@dim]
        fft_size: Target fft_size, should be greater than ccep_size.
    Returns:
        impulse_responses: [fft_size@dim]. Approximated time wrapped impulse responses.
    """
...
def complex_cepstrum_lowpass_mask(ccep_size: int, max_quefrency: int, device: Optional[torch.device] = "cuda") -> Tensor:
...
def complex_cepstrum_scale(ccep_size: int, device: Optional[torch.device] = "cuda") -> Tensor:
    """Generate 1 / |n| scale for scaling complex cepstrums.
    Returns:
        scale: shape [n].
    """
    meter = torch.linspace(-(ccep_size // 2), (ccep_size - 1) // 2, ccep_size).abs()
    meter = torch.cat([meter[ccep_size // 2:], meter[: ccep_size // 2]])  # TODO
    meter[0] = 1.0
